...
void test_session_is_valid_within_timeout(void) {
    ...
    g_protocol_state.session_start_timestamp = 1000000;
...
void test_session_timestamp_tracking(void) {
    ...
    TEST_ASSERT_EQUAL_UINT64(expected_time, g_protocol_state.session_start_timestamp);
}
...
void test_trigger_reattestation_generates_new_ephemeral_key(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_new_key, g_protocol_state.et_pubkey, 64);
}
...
void test_session_custom_timeout(void) {
    ...
    g_protocol_state.session_valid = true;
    g_protocol_state.session_timeout_ms = 60000;
    g_protocol_state.session_start_timestamp = 1000000;
...
void test_heartbeat_resets_missed_count(void) {
    ...
    g_protocol_state.current_state = 0x40;
    g_protocol_state.missed_hb_count = 3;
    ...
    TEST_ASSERT_EQUAL_UINT64(5000000, g_protocol_state.last_hb_timstamp);
}
...
void test_three_missed_heartbeats_should_trigger_shutdown(void) {
    ...
    bool should_shutdown = (g_protocol_state.missed_hb_count >= 3);
...
void test_two_missed_heartbeats_no_shutdown(void) {
    ...
    g_protocol_state.current_state = 0x40;
    g_protocol_state.session_valid = true;
    g_protocol_state.missed_hb_count = 2;
...
void test_missed_heartbeat_counter_increments(void) {
    ...
    g_protocol_state.current_state = 0x40;
    g_protocol_state.session_valid = true;
    g_protocol_state.missed_hb_count = 0;
    uint8_t initial_count = g_protocol_state.missed_hb_count;
    
    // Act: Simulate missed heartbeat (increment counter)
    g_protocol_state.missed_hb_count++;
    
    // Assert: Verify counter incremented
    TEST_ASSERT_EQUAL_UINT8(initial_count + 1, g_protocol_state.missed_hb_count);
}
...
void test_halt_state_sets_flag(void) {
    ...
    g_protocol_state.in_halt_state = false;
    ...
    TEST_ASSERT_TRUE(g_protocol_state.in_halt_state);
